...
async def _send_queued_email_async(campaign_contact_id: uuid.UUID) -> None:
    ...
    if not await acquire_send_lock(redis, campaign_contact_id):
        logger.info("send_lock_not_acquired_skipping", campaign_contact_id=str(campaign_contact_id))
        return
    ...
    async with AsyncSessionLocal() as db:
        ...
        # Race guard: something else (a manual skip/suppress, a concurrent tick) may
        # have moved this contact on since it was scheduled.
        ...
        if draft is None or not draft["quality_gate"]["passed"]:
            # Shouldn't happen — approve() already enforced this — but never send
            # something that failed its own quality gate.
            logger.error("send_blocked_failed_quality_gate", campaign_contact_id=str(campaign_contact_id))
            return
        ...
        request = SendEmailRequest(
            ...
            in_reply_to_message_id=prior_message.provider_message_id if prior_message else None,
        )
